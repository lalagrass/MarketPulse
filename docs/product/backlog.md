...
| `data/raw` 可驗證清單（session date・校驗和・判定：交易日／假日／抓取失敗） | ENG | 011 階段 1 | 836MB／882 檔、`.gitignore` 排除、無備份、不可重新產生（33 天已凍結成空檔）。004 的 `retained_fraction 69.25%` 就是在這份資料上算的。需自己一輪 |
