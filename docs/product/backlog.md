...
| radar 表格 `Momentum` 欄沒有欄寬 | ENG | 009 驗收・**011 惡化** | 009 DO-3.1 只給 `敘事` 補了右側空白（最後一欄，畫面零改變），真正的病灶是 `Momentum` 標籤長度 4–9 讓 `敘事` 欄左緣浮動。**011 DO-3 把那一格從 4–9 格拉到 22–27 格**（`radar.py:154-163` `_fmt_mom_ascii` 無欄寬），`敘事` 左緣浮動再加 18 格位移，`"-" * _vislen(header)` 分隔線也短於資料列。物證見 `011-report.md` 第 4 節。**排 012 第一格** |
...
| radar 新鮮度改成「距今差幾個交易日」 | 使用 | 010 驗收 | **011 降級為 UNKNOWN。****立案物證是錯的**——010-report 寫「09-05 週五是交易日」，但 2026-09-04 是週五、09-05 是週六，當天資料並未落後（`011-evidence.md` 第 1 節）。`caught_up` 偵測不到「根本沒跑過 refresh」這個結構性論點仍成立，但沒有真實案例；且研究證實 `pandas_market_calendars` 無台灣日曆。**回頭日期：012 驗收後**——屆時 011 DO-2 已讓「這一天到底發生什麼」可區分，再決定要不要做 |
...
| A4 那一行改用實際漲跌停價比對 | 使用 | 011 驗收 | 現在讀作「近 20 個交易日 105 筆」，其中絕大多數是 ±10.0% 的漲跌停檔位進位（全樣本 1644/1909 = 86%），6669 被埋在裡面。**檔位表也是交易所規則，不是 D10 把手**——用規則的完整形式取代 0.10 這個簡化版 |
