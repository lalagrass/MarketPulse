...
*[加入 2026-09-08。§26 的 MVP 不含本節；第二層自 sprint 001 起逐輪長出來，
但一直沒有寫進本文件——2026-09-08 grep 全文只有 §11 一句不相干的帶過。
本節補上產品意圖；實作細節仍以各輪 `docs/sprints/NNN-spec.md` 為準。]*

## 28.1 兩層的介面

第一層是一個服務：**給它一組代號，它回答這組代號相對大盤的強弱**——
RS5／RS20／RS60、breadth、value_share，成員照 as-of 讀。

**第二層的唯一輸出就是籃子。**任何能生出一群代號的方法都可以接上來：podcast、
法說會、貼文、人的判斷。第二層自己不產生數字。

`non-goals.md` R3 是這個介面的規則形式：故事可以決定「要量哪一組股票」，
不能改變「怎麼量」。

**代價要寫在旁邊：籃子的品質決定一切，而第一層不會告訴你分錯了。**
一組亂寫的代號照樣算得出漂亮的 RS20。唯一的防線是把 `n`、`breadth`、
重疊檔數印在數字旁邊。

## 28.2 一則故事的形狀

一則 narrative 底下有若干條 **branch（支線）**，每條支線有一個可證偽的 `claim`。
每條支線掛三個籃子，**並列，不相減，不排名，不進主排名池**：

| 籃子 | 它跟的是 | 內容 |
|---|---|---|
| `if_true` — 成真受惠 | 誰贏 | 代號 |
| `if_false` — 反面受惠 | 誰贏 | 代號 |
| `either_way` — 誰贏都賺 | **有沒有人在花錢** | `theme_ids` |

第二籃是「對立面受惠」，**不是「受害」**——很多故事不是零和（XPU 興起與 GPU 廠
可以是 TAM 一起膨脹）。

第三籃跟的是資本支出，不是勝負，所以它是這條故事的**生命跡象**。
沒有它，「兩面都弱」分不出是故事死了、還是錢照花而勝負未定。

## 28.3 怎麼讀

**兩步，而且那是給人的閱讀順序，不是程式的分支。**
面板不做強弱判定，只按固定順序把三行排出來，並在表頭印一次字面鎖定的讀法短註。

1. 先看「誰贏都賺」——這條故事還有沒有人在花錢
2. 再看「成真」與「反面」的相對位置——市場往哪邊走

**門檻一律不設（D10）。**「強」與「弱」由讀的人自己定，不由程式定。

## 28.4 紀律

- **帶日期快照，不原地修改。**分組寫錯了就開新快照，讓錯誤留在歷史上。
  這是第二層唯一的防偽線——第一層錯了會被虛無檢定抓到，第二層錯了不會有任何
  東西出聲，因為每一種讀法事後都說得通。
- **回頭日期必填，而且必須是一個日期。**條件寫在 `revisit_note`。
  「不知道什麼時候會知道」的正確處理是挑一個保守日期、到期再延；延期本身也是資料。
- **人講的與我們推的分開存放。**`named_symbols` / `inferred_symbols`。
  外部依據見 `non-goals.md` D18。
- **回看由人做，程式最多提醒到期。**見 R3 的具體反例。

## 28.5 第二層的成功判準

§27 給第一層的判準是「一眼看出月份 → 當月贏家」。第二層的對應版本是：

打開輸出，對任何一則還活著的故事，能在五秒內回答——
...
綜合評分、信心分數、熱度／爆發偵測、自動抽取事件、程式自動判定支線成立與否、
籃子之間的價差線、用統計分群決定籃子成員。
理由與規則帳見 `docs/product/non-goals.md` R1／R3／D10／D17／D18。
